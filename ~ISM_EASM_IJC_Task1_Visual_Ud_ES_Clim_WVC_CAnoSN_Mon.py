...
with xr.open_dataset('C:/~Dataset/Clim_ERA5_WVC_250hPa_1X1_1998_To_2019.nc') as dta:
...
oup_ano12 = './Submission/IJC/Fig_&_Table/ES_%s_%s_%s_d%s_P%s_MSSD_ERA5VC12_Day[%s]_Mon.png' % (PERIOD.split('.')[0], SEASON, str(
    PERCENTILE).split('.')[0] + str(PERCENTILE).split('.')[1], str(TAUMAX), str(SIGNIFICANCE).split('.')[0] + str(SIGNIFICANCE).split('.')[1], str(DAY))
oup_ano21 = './Submission/IJC/Fig_&_Table/ES_%s_%s_%s_d%s_P%s_MSSD_ERA5VC21_Day[%s]_Mon.png' % (PERIOD.split('.')[0], SEASON, str(
    PERCENTILE).split('.')[0] + str(PERCENTILE).split('.')[1], str(TAUMAX), str(SIGNIFICANCE).split('.')[0] + str(SIGNIFICANCE).split('.')[1], str(DAY))
oup_anoboth = './Submission/IJC/Fig_&_Table/ES_%s_%s_%s_d%s_P%s_MSSD_ERA5VCboth_Day[%s]_Mon.png' % (PERIOD.split('.')[0], SEASON, str(
    PERCENTILE).split('.')[0] + str(PERCENTILE).split('.')[1], str(TAUMAX), str(SIGNIFICANCE).split('.')[0] + str(SIGNIFICANCE).split('.')[1], str(DAY))
...
for i, l in enumerate(lags):
    ...
    if l == DAY:
        ax_list = [ax1, ax2]
    if ax_list != []:
        v_list = [swano[1, i], nwano[1, i]]
        box_list = [[regional_box('ARBSEA'), regional_box('SCN1')],
                    [regional_box('CMZ'), regional_box('NCN')]]
        pre_list = ['250', '250']
        for ax, v, box, pre in zip(ax_list, v_list, box_list, pre_list):
            bm = Basemap(ax=ax,
                         projection='mill',
            ...
            cb_bm = bm.colorbar(cf_bm, location='bottom',
                                ticks=np.arange(-4, 4.1, 1),
                                pad=.2, size=.1)
            cb_bm.ax.set_xlabel(r'$v$ (%shPa) (m s$^{-1}$)' % pre,
                                family=fnt.get_family(),
                                fontsize=fnt.get_size(),
                                fontweight=fnt.get_weight(),
                                fontstretch=fnt.get_stretch())

...
for PRESSURE in PRESSURELEVEL_LIST:
    inp_clim_wvc = 'C:/~Dataset/Clim_ERA5_WVC_%shPa_1X1_1998_To_2019.nc' % (
        PRESSURE)
    inp_clim_wuc = 'C:/~Dataset/Clim_ERA5_WUC_%shPa_1X1_1998_To_2019.nc' % (
        PRESSURE)
    with xr.open_dataset(inp_clim_wvc) as dta:
        lat = dta['latitude'].values.astype(np.float32)
        lon = dta['longitude'].values.astype(np.float32)
        ttl = dta['time'].values
        v = dta['v']

    with xr.open_dataset(inp_clim_wuc) as dta:
        u = dta['u']
    ...
    for m in range(1, 13):
        ...
        tv = dict(v.groupby('time.month'))[
            m].values - np.mean(dict(v.groupby('time.month'))[m].values, axis=0)
        tu = dict(u.groupby('time.month'))[
            m].values - np.mean(dict(u.groupby('time.month'))[m].values, axis=0)
        vano[idx, :, :] = tv
        uano[idx, :, :] = tu
    del v, u
    ...
    for REGION in REGION_LIST:
        ...
        oup_ano = './Results/%s/ES_%s_%s_%s_d%s_P%s_MSSD_ERA5%sWVCAno[%s-%s]_Mon.npy' % (DATASETS, PERIOD.split('.')[0], SEASON, str(
        ...
        if SPAZOOMOUT != 1:
            oup_ano = './Results/%s/ES_%s_%s_%s_d%s_P%s_X%s_MSSD_ERA5%sWVCAno[%s-%s]_Mon.npy' % (DATASETS, PERIOD.split('.')[0], SEASON, str(PERCENTILE).split('.')[0] + str(
        ...
        for i, l in enumerate(lags):
            print("Lag: ", l)
            vt = vano[(syncd12 + l).astype(np.int32)]
            ut = uano[(syncd12 + l).astype(np.int32)]
            ws_ano = np.mean(np.sqrt(vt ** 2 + ut ** 2), axis=0)
            v_ano = np.mean(vt, axis=0)
            u_ano = np.mean(ut, axis=0)
            sync_ano[0, i] = ws_ano
            sync_ano[1, i] = v_ano
            sync_ano[2, i] = u_ano
            vt = vano[(syncd21 + l).astype(np.int32)]
            ut = uano[(syncd21 + l).astype(np.int32)]
            ws_ano = np.mean(np.sqrt(vt ** 2 + ut ** 2), axis=0)
            v_ano = np.mean(vt, axis=0)
            u_ano = np.mean(ut, axis=0)
            sync_ano[3, i] = ws_ano
            sync_ano[4, i] = v_ano
            sync_ano[5, i] = u_ano
            vt = vano[(both + l).astype(np.int32)]
            ut = uano[(both + l).astype(np.int32)]
            ws_ano = np.mean(np.sqrt(vt ** 2 + ut ** 2), axis=0)
            v_ano = np.mean(vt, axis=0)
            u_ano = np.mean(ut, axis=0)
            sync_ano[6, i] = ws_ano

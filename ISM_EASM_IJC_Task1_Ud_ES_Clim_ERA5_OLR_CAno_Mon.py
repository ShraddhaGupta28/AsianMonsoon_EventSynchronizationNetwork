...
inp_clim_gph = 'C:/~Dataset/Clim_ERA5_OLR_1X1_1998_To_2019.nc'
with xr.open_dataset(inp_clim_gph) as dta:
    ...
    ttl = dta['time'].values
    ttl = np.arange(ttl.shape[0]).astype(np.uint16)
    z = dta['mtnlwrf']
...
for REGION in REGION_LIST:
    ...
    oup_ano = './Results/%s/ES_%s_%s_%s_d%s_P%s_MSSD_ERA5OLRCAno[%s-%s]_Mon.npy' % (DATASETS, PERIOD.split('.')[0], SEASON, str(
        PERCENTILE).split('.')[0] + str(PERCENTILE).split('.')[1], str(TAUMAX), str(SIGNIFICANCE).split('.')[0] + str(SIGNIFICANCE).split('.')[1], str(REGION[0]), str(REGION[1]))
    if SPAZOOMOUT != 1:
        oup_ano = './Results/%s/ES_%s_%s_%s_d%s_P%s_X%s_MSSD_ERA5OLRCAno[%s-%s]_Mon.npy' % (DATASETS, PERIOD.split('.')[0], SEASON, str(PERCENTILE).split('.')[0] + str(
